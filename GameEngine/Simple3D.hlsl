...
VS_OUT VS(float4 pos : POSITION,float4 uv : TEXCOORD,float4 normal : NORMAL, float4 tangent : TANGENT)
{
	...
	float4 eye = normalize(mul(pos,matW)-g_cameraPosition);
...
float4 PS(VS_OUT inData) : SV_Target
{
	...
	float4 normal_01 = g_normalTexture.Sample(g_sampler, inData.uv) * 2 - 1;
	float4 normal_02 = g_normalTexture.Sample(g_sampler, inData.uv) * 2 - 1;
	...
	{
		diffuse = g_diffuseColor*inData.col;
	}
